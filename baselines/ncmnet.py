...
def knn(x, k):
    ...
    xx = torch.sum(x**2, dim=1, keepdim=True)  # B1N
...
class DS_Block(nn.Module):
    ...
    def forward(self, x, y):
        if y is None:
            xs, out = x[1], x[0].transpose(1, 2).unsqueeze(dim=-1)
            B, _, N, _ = xs.size()
            xs = xs.transpose(1, 3).contiguous()
        else:
            B, _, N, _ = x.size()
            xs = x.transpose(1, 3).contiguous()
            out = self.conv(xs)  # BCN1

        idx_sn = knn(xs.squeeze(-1), k=self.k_num)  # BNK
        ...
        if y is None:
            return out.transpose(1, 2).squeeze(dim=-1)
