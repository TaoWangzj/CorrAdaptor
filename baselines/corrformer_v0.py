...
class DS_Block(nn.Module):
    def __init__(self, config, initial=False, predict=False, out_channel=128, k_num=8, sampling_rate=0.5):
        ...
        self.ltr1 = LinearTransformers(self.out_channel, 4, ['self', 'self'])  # recommend a change to 1
        self.ltr2 = LinearTransformers(self.out_channel, 4, ['self', 'self'])
    ...
    def forward(self, x, y):
        ...
        # stack 2 times
        local_graphs = get_graph_feature(x, k=self.k_num)  # explict local graphs
        local_graphs = self.gab1(local_graphs)
        local_context = self.aggregator1(local_graphs)
        gl_context = self.ltr1(self.trans_shape(x, "in"), self.trans_shape(local_context, "in"))
        gl_context = self.trans_shape(gl_context, "out")  # global & local context
        w0 = self.linear_0(gl_context).view(B, -1)

        local_graphs = get_graph_feature(gl_context, k=self.k_num)
        local_graphs = self.gab2(local_graphs)
        local_context = self.aggregator2(local_graphs)
        gl_context = self.ltr2(self.trans_shape(gl_context, "in"), self.trans_shape(local_context, "in"))
        ...
        if self.predict == False:
            ...
            x_ds, y_ds, w0_ds = self.down_sampling(origin_x, y, w0, indices, None, self.predict)
            return x_ds, y_ds, [w0, w1], [w0_ds, w1_ds]
        else:
            w1_ds, indices = torch.sort(w1, dim=-1, descending=True)
            w1_ds = w1_ds[:, :int(N*self.sr)]
            x_ds, y_ds, w0_ds, out = self.down_sampling(origin_x, y, w0, indices, gl_context, self.predict)
...
class CLNet(nn.Module):
    def __init__(self, config):
        ...
        self.ds_1 = DS_Block(config, initial=False, predict=True, out_channel=128, k_num=9, sampling_rate=0.5)
