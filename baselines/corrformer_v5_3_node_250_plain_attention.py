...
# 利用 Self-attention 和 Cross-attention 进行交互
class Flow_Attention(nn.Module):
    """
    A standard multi-head self-attention module with query, key, and value transformations.
    """
...
class My_BlockLayerScale(nn.Module):
    def __init__(self, dim, nhead, mlp_ratio=4., drop=0., drop_path=0., act_layer=nn.GELU,
        ...
        self.self_attn = Flow_Attention(dim, nhead)
        self.cross_attn = Flow_Attention(dim, nhead)
    ...
    def forward(self, q, kv, motion):
        ...
        # MHSA
        q = q + self.drop_path(self.self_attn(x, y))
        q = self.norm2(q)
        # MHCA
        q = q + self.drop_path(self.cross_attn(q, motion))
        q = self.norm3(q)
        q = q + self.drop_path(self.gamma * self.mlp(q))
        return q
...
class DS_Block(nn.Module):
    def __init__(self, config, initial=False, predict=False, out_channel=128, k_num=8, sampling_rate=0.5):
        ...
        self.pool1 = diff_pool(out_channel, 250)
        self.pool2 = diff_pool(out_channel, 250)
        self.oafilter1 = nn.Sequential(OAFilter(out_channel, 250),
                                       OAFilter(out_channel, 250),
                                       OAFilter(out_channel, 250))
        self.oafilter2 = nn.Sequential(OAFilter(out_channel, 250),
                                       OAFilter(out_channel, 250),
                                       OAFilter(out_channel, 250))
        self.unpool1 = diff_unpool(out_channel, 250)
        self.unpool2 = diff_unpool(out_channel, 250)
