...
class My_BlockLayerScale(nn.Module):
    ...
    def forward(self, q, kv):
        x = self.norm1_1(q)
        y = self.norm1_2(kv)
        q = q + self.drop_path(self.attn(x, y))
        x = self.norm2(q)
...
class DS_Block(nn.Module):
    def __init__(self, config, initial=False, predict=False, out_channel=128, k_num=8, sampling_rate=0.5):
        ...
        self.ltr11 = LinearTransformers(self.out_channel, 4, ['self', 'cross'] * 2)  # recommend a change to 1
        self.ltr2 = LinearTransformers(self.out_channel, 4, ['self', 'self'] * 2)
        self.ltr22 = LinearTransformers(self.out_channel, 4, ['self', 'cross'] * 2)  # recommend a change to 1
    ...
    def forward(self, x, y):
        ...
        e_local_context = self.ltr11(self.trans_shape(e_local_context, "in"), self.trans_shape(i_local_context, "in"))
        e_local_context = self.trans_shape(e_local_context, "out")  # global & local context

        e_gl_context = self.ltr1(self.trans_shape(x, "in"), self.trans_shape(e_local_context, "in"))

        # gl_context = self.ltr1(self.trans_shape(i_local_context, "in"), self.trans_shape(e_local_context, "in"))

        gl_context = self.trans_shape(e_gl_context, "out")  # global & local context
        # fusion
        ...
        e_local_context = self.ltr22(self.trans_shape(e_local_context, "in"), self.trans_shape(i_local_context, "in"))
        e_local_context = self.trans_shape(e_local_context, "out")  # global & local context

        gl_context = self.ltr2(self.trans_shape(gl_context, "in"), self.trans_shape(e_local_context, "in"))
