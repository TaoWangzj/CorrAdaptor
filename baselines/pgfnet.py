...
class GRAModule(nn.Module):
    def __init__(self, net_channels, input_channel, depth, clusters):
        ...
        self.pretrain_conv1 = nn.Conv2d(channels//2, channels, kernel_size=1)
        self.pretrain_conv2 = nn.Conv2d(channels, channels//2, kernel_size=1)
...
class PGFNet(nn.Module):
    def __init__(self):
        ...
        self.weights_init = GRAModule(256, 4 + self.side_channel, depth_each_stage, 500)
        self.weights_iter = [GRAModule(256, 6 + self.side_channel, depth_each_stage, 500) for
                             _ in range(1)]
    ...
    def forward(self, xs):
        ...
        if self.side_channel > 0:
            sides = xs.transpose(1, 2).unsqueeze(3)
        ...
        data_new = torch.stack(
            [xs[i].squeeze(0)[index_k[i]] for i in range(input.size(0))]).unsqueeze(1)
        ...
        for i in range(self.iter_num):
            ...
            e_hat = weighted_8points(xs, logits_2)
