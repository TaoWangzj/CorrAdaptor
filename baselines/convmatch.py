...
class ConvMatch(nn.Module):
    def __init__(self, use_gpu=True):
        ...
        self.grid_center = GridPosition(16, use_gpu=use_gpu)
        self.pos_embed = PositionEncoder(128)
        self.grid_pos_embed = PositionEncoder(128)
        self.init_project = InitProject(128)
        self.layer_blocks = nn.Sequential(
            *[LayerBlock(128, 4, 16) for _ in range(self.layer_num)]
        )
    ...
    def forward(self, xs):
        ...
        for i in range(self.layer_num):
            d, logits, e_hat = self.layer_blocks[i](xs, d, grid_pos_embed)  # BCN
