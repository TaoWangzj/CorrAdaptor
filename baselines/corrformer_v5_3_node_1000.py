...
class DS_Block(nn.Module):
    def __init__(self, config, initial=False, predict=False, out_channel=128, k_num=8, sampling_rate=0.5):
        ...
        self.pool1 = diff_pool(out_channel, 1000)
        self.pool2 = diff_pool(out_channel, 1000)
        self.oafilter1 = nn.Sequential(OAFilter(out_channel, 1000),
                                       OAFilter(out_channel, 1000),
                                       OAFilter(out_channel, 1000))
        self.oafilter2 = nn.Sequential(OAFilter(out_channel, 1000),
                                       OAFilter(out_channel, 1000),
                                       OAFilter(out_channel, 1000))
        self.unpool1 = diff_unpool(out_channel, 1000)
        self.unpool2 = diff_unpool(out_channel, 1000)
