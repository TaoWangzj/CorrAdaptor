...
class Flow_Attention(nn.Module):
    ...
    def forward(self, q, kv, motion):
        ...
        m_q = self.m_proj(motion).reshape(B_, N, self.num_heads, C // self.num_heads).transpose(1, 2)
        m_k = self.m_proj(motion).reshape(B_, N, self.num_heads, C // self.num_heads).transpose(1, 2)
        m_v = self.m_proj(motion).reshape(B_, N, self.num_heads, C // self.num_heads).transpose(1, 2)
        # q, k, v = qkv[0], qkv[1], qkv[2]
        # kernel
        q, k, m_q, m_k = self.kernel(q), self.kernel(k), self.kernel(m_q), self.kernel(m_k)
        # motionformer
        # m -> q; q -> q, k -> m_q, v -> m_v
        sink_incoming_q = 1.0 / (self.my_sum(q + 1e-6, m_q.sum(dim=2) + 1e-6) + 1e-6)
        source_outgoing_q = 1.0 / (self.my_sum(m_q + 1e-6, q.sum(dim=2) + 1e-6) + 1e-6)
        conserved_sink_q = self.my_sum(q + 1e-6, (m_q * source_outgoing_q[:, :, :, None]).sum(dim=2) + 1e-6) + 1e-6
        conserved_source_q = self.my_sum(m_q + 1e-6, (q * sink_incoming_q[:, :, :, None]).sum(dim=2) + 1e-6) + 1e-6
        conserved_source_q = torch.clamp(conserved_source_q, min=-1.0, max=1.0)  # for stability
        # m -> q allocation
        sink_allocation_q = torch.sigmoid(conserved_sink_q * (float(q.shape[2]) / float(m_q.shape[2])))
        # m -> q competition
        source_competition_q = torch.softmax(conserved_source_q, dim=-1) * float(m_q.shape[2])
        # m -> q multiply
        kv_q = m_q.transpose(-2, -1) @ (m_v * source_competition_q[:, :, :, None])
        x_update_q = ((q @ kv_q) * sink_incoming_q[:, :, :, None]) * sink_allocation_q[:, :, :, None]
        x_q = (x_update_q).transpose(1, 2).reshape(B_, N, C)
        x_q = self.proj_q(x_q).reshape(B_, N, self.num_heads, C // self.num_heads).transpose(1, 2)
        # m -> k; q -> k, k -> m_k, v -> m_v
        sink_incoming_k = 1.0 / (self.my_sum(k + 1e-6, m_k.sum(dim=2) + 1e-6) + 1e-6)
        source_outgoing_k = 1.0 / (self.my_sum(m_k + 1e-6, k.sum(dim=2) + 1e-6) + 1e-6)
        conserved_sink_k = self.my_sum(k + 1e-6, (m_k * source_outgoing_k[:, :, :, None]).sum(dim=2) + 1e-6) + 1e-6
        conserved_source_k = self.my_sum(m_k + 1e-6, (k * sink_incoming_k[:, :, :, None]).sum(dim=2) + 1e-6) + 1e-6
        conserved_source_k = torch.clamp(conserved_source_k, min=-1.0, max=1.0)  # for stability
        # m -> k allocation
        sink_allocation_k = torch.sigmoid(conserved_sink_k * (float(k.shape[2]) / float(m_k.shape[2])))
        # m -> k competition
        source_competition_k = torch.softmax(conserved_source_k, dim=-1) * float(m_k.shape[2])
        # m -> k multiply
        kv_k = m_k.transpose(-2, -1) @ (m_v * source_competition_k[:, :, :, None])
        x_update_k = ((k @ kv_k) * sink_incoming_k[:, :, :, None]) * sink_allocation_k[:, :, :, None]
        x_k = (x_update_k).transpose(1, 2).reshape(B_, N, C)
        x_k = self.proj_k(x_k).reshape(B_, N, self.num_heads, C // self.num_heads).transpose(1, 2)
        # info injection
        q = q + q * x_q
        k = k + k * x_k
        # normalizer
...
class DS_Block(nn.Module):
    ...
    def forward(self, x, y):
        ...
        # implict branch
        i_local_graphs = self.pool1(x)  # output -> [B, 128, 500, 1]
        i_local_graphs = self.oafilter1(i_local_graphs)
        i_local_context = self.unpool1(x, i_local_graphs)
        ...
        # explict branch
        e_local_graphs = get_graph_feature(x, k=self.k_num)  # explict local graphs
        e_local_graphs = self.gab1(e_local_graphs)
        e_local_context = self.aggregator1(e_local_graphs)
        # e_gl_context = self.ltr1(self.trans_shape(x, "in"), self.trans_shape(e_local_context, "in"))

        gl_context = self.ltr1(self.trans_shape(i_local_context, "in"), self.trans_shape(e_local_context, "in"), motion)

        gl_context = self.trans_shape(gl_context, "out")  # global & local context
        # fusion
        # gl_context = self.fusion1(torch.cat([i_gl_context, e_gl_context], dim=1))
        w0 = self.linear_0(gl_context).view(B, -1)
        ...
        if self.predict == False:
            ...
            x_ds, y_ds, w0_ds = self.down_sampling(initial_x, y, w0, indices, None, self.predict)
            return x_ds, y_ds, [w0, w1], [w0_ds, w1_ds]
        else:
            w1_ds, indices = torch.sort(w1, dim=-1, descending=True)
            w1_ds = w1_ds[:, :int(N*self.sr)]
            x_ds, y_ds, w0_ds, out = self.down_sampling(initial_x, y, w0, indices, gl_context, self.predict)
