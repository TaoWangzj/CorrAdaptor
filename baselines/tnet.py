...
class T_Net(nn.Module):
    def __init__(self):
        ...
        self.subnetwork_init = sub_T(128, 4, depth_each_stage
                                     , 500)
        self.subnetwork = [sub_T(128, 6, depth_each_stage, 500) for _ in
                           range(self.iter_num)]
        self.subnetwork = nn.Sequential(*self.subnetwork)
        self.l = []
        self.l.append(PCSE((self.iter_num + 1) * 128, 128 // 2, 1))
        for _ in range(3):
            self.l.append(PCSE(128, 128 // 2, 1))
    ...
    def forward(self, xs):
        ...
        logits, e_hat, residual, out = self.subnetwork_init(input, xs)
        More_weight = out
        res_weights.append(logits), res_e_hat.append(e_hat)
        for i in range(self.iter_num):
            logits, e_hat, residual, out = self.subnetwork[i](
                torch.cat([input, residual.detach(), torch.relu(torch.tanh(logits)).reshape(residual.shape).detach()], dim=1), xs)
        ...
        return res_weights, res_e_hat, residual[:, 0, :, 0]
