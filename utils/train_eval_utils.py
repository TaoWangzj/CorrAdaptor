...
from baselines.loss1 import MatchLoss as MatchLoss1  # such as clnet, mgnet, and ncmnet
from baselines.loss2 import MatchLoss as MatchLoss2  # such as oanet, convmatch, ms2dgnet, msanet, pgfnet, tnet, and umatch
...
def train_one_epoch(model, optimizer, data_loader, conf, device, epoch, cur_global_step, **kwargs):
    ...
    loss_function1 = MatchLoss1(conf)
    loss_function2 = MatchLoss2(conf)
    ...
    for step, data in enumerate(data_loader):
        ...
        if conf.series_type == "clnet":  # such as clnet, mgnet, and ncmnet
            res_logits, ys_ds, res_e_hat, _, xs_ds = model(xs, ys)
            
            if res_e_hat[-1] is None:
                optimizer.zero_grad()
                del res_logits, res_e_hat
                continue

            loss, ess_loss, classif_loss = loss_function1.run(cur_global_step, data, res_logits, ys_ds, [res_e_hat[-1]])
            loss.backward()
            ...
            if is_main_process():
                data_loader.desc = "[Training epoch {}] LR: {}, loss: {}, ess_loss: {}, classif_loss: {}, inlier_ratio: {}, ". \
                    format(epoch, cur_lr, round(mean_loss.item(), 3), round(mean_eloss.item(), 3),
                           round(mean_closs.item(), 3), round(mean_inlier.item(), 3))
                kwargs['tb_writer'].add_scalar("train_loss", mean_loss.item(), cur_global_step + step)
        else:  # such as oanet, convmatch, ms2dgnet, msanet, pgfnet, tnet, and umatch
            res_logits, res_e_hat, _ = model(xs)
            ...
            for i in range(len(res_logits)):
                loss_i, geo_loss, cla_loss, l2_loss = loss_function2.run(cur_global_step, data, res_logits[i], res_e_hat[i])
                loss += loss_i
                ess_loss += geo_loss
                classif_loss += cla_loss

            loss.backward()

            with torch.no_grad():
                is_pos = (ys < conf.obj_geod_th).type(ys.type())
                is_neg = (ys >= conf.obj_geod_th).type(ys.type())
                inlier_ratio = torch.sum(is_pos, dim=-1) / (torch.sum(is_pos, dim=-1) + torch.sum(is_neg, dim=-1))
                inlier_ratio = inlier_ratio.mean().item()
...
def evaluate(model, data_loader, config, epoch):
    ...
    for step, test_data in enumerate(data_loader):
        ...
        if config.series_type == 'clnet':
            res_logits, ys_ds, res_e_hat, y_hat, xs_ds = model(xs, ys)
